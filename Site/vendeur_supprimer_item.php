...
<html>
	<head>
		<title>ECE Market Place | Vendeur Supprimer Items</title>
		<meta charset="utf-8"/>
		...
		<!-- Page Content -->
			...
			<div id="page-content-wrapper">
				<div class="container-fluid">
					...
					<h1 style="font-weight: bold; text-align: center;">Choisir la quantité a supprimer de votre stock de chaque article</h1><br>
					<!-- Recherche -->
					<div style="text-align: center">
						<form action="vendeur_supprimer_item.php" method="post">
						...
						<form action="vendeur_suppression_item.php" method="post">	
							...
							<tbody>
								...
								if($db_found){	
									if(empty($research)){
										...
										$sql = "SELECT * FROM vendre INNER JOIN item ON item.Id_item = vendre.Id_item INNER JOIN imgitem ON item.Id_item = imgitem.Id_item WHERE imgitem.Is_main = '1' AND vendre.Email_ECE ='".$_SESSION['Email_ECE']."' ORDER BY item.quantite DESC "; 
										...
										// on select dans la base
										$sql = "SELECT * FROM vendre INNER JOIN item ON item.Id_item = vendre.Id_item INNER JOIN imgitem ON item.Id_item = imgitem.Id_item WHERE imgitem.Is_main = '1' AND vendre.Email_ECE ='".$_SESSION['Email_ECE']."' AND (item.nom = '".$research."' OR  item.categorie = '".$research."') ORDER BY item.quantite DESC "; 
										$req = mysqli_query($db_handle, $sql); 
										while($data=mysqli_fetch_assoc($req))
										{
											echo("<tr>");
